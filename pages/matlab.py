...
def _load_cert(index: int) -> str:
    return f"certificates/cert{index}.jpg"
...
class MatlabPage:
    COURSES = [
        ...
        {"title": "Make and manipulate matrices", "description": "Creation, indexing, and mathematical operation of multi-dimensional arrays and vectors in MATLAB.",
         "date": "24 Jan 2026", "hours": "~2 hrs", "cert_image": _load_cert(5)},
        {"title": "Calculation with vectors and matrices", "description": "Filtering, FFT, and frequency-domain analysis in MATLAB.",
        ...
        return _hoverable(
            ft.Container(
                ...
                content=ft.Column(controls=[
                    ...
                    ft.Container(
                        content=ft.Image(
                            src=course["cert_image"], fit="contain",
                            error_content=ft.Column(controls=[
                                ft.Icon(ft.Icons.WORKSPACE_PREMIUM, color=ACCENT_B, size=28),
                                ft.Text("Certificate pending upload", size=11, color=TEXT_SEC,
                                        text_align=ft.TextAlign.CENTER, italic=True),
                            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=4),
                        ),
                        bgcolor=SURFACE2, border_radius=8, height=200,
                        padding=ft.Padding(left=8, right=8, top=8, bottom=8),
                        border=ft.Border.all(1, BORDER),
                        margin=ft.Margin(left=0, right=0, top=4, bottom=0),
                        clip_behavior=ft.ClipBehavior.HARD_EDGE,
                        on_click=lambda e, src=course["cert_image"]: self._open_preview(e, src),
                        ink=True, tooltip="Click to zoom",
                    ),
                ], spacing=8),
